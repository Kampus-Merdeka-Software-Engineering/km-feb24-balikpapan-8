...
function displayCharts(filteringProduct, filteringYears, filteringLineYears) {
    ...
    fetch("./data/productCategory.json")
        ...
        .then((data) => {
            ...
            // Aggregate yearly revenue and profit data
            ...
            const revenueData = yearlyRevenue.filter(item => item.label === "Revenue");
            const profitData = yearlyRevenue.filter(item => item.label === "Profit");

            const lineLabels = revenueData.map((item) => item.year);
            const lineRevenueData = revenueData.map((item) => item.revenue);
            const lineProfitData = profitData.map((item) => item.profit);
