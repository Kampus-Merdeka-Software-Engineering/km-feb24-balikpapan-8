...
const inputCheckboxYears = filteringByYears.querySelectorAll("input[type='checkbox']");
const filteringByLineYears = document.getElementById("filteringByYear");
const inputCheckboxLineYears = filteringByLineYears.querySelectorAll("input[type='checkbox']");
    ...
    fetch("./data/productCategory.json")
        ...
        .then((data) => {
            ...
            const filteredByYears = data.productCategory.filter(
                (item) => filteringYears.length === 0 || filteringYears.includes(item.year)
            );
            ...
            if (filteringYears.includes("2015")) {
                aggregatedData2015 = filteredByYears.filter((item) => item.year === "2015");
            }

            if (filteringYears.includes("2016")) {
                aggregatedData2016 = filteredByYears.filter((item) => item.year === "2016");
            }
            ...
            const labelsProductCategory = aggregatedDataByCategory.map((cat) => cat.label);
            const revenue2015 = aggregatedDataByCategory.map((cat) => cat["2015"] || 0);
            const revenue2016 = aggregatedDataByCategory.map((cat) => cat["2016"] || 0);
            ...
            if (filteringLineYears.length) {
                yearlyRevenue = yearlyRevenue.filter(item => filteringLineYears.includes(item.year));
            }
