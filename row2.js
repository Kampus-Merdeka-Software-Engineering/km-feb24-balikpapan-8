...
function displayChartProductCategory(filteringProduct, filteringYears) {
    ...
    fetch("./data/productCategory.json")
        ...
        .then((data) => {
            ...
            const filteredByYears = data.productCategory.filter(item =>
            ...
            const revenue2015 = aggregatedDataByCategory.map(cat => cat["2015"] || 0); // Use 0 if revenue data is not available
            const revenue2016 = aggregatedDataByCategory.map(cat => cat["2016"] || 0); // Use 0 if revenue data is not available
