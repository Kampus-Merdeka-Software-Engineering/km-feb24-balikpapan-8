document.addEventListener("DOMContentLoaded", function() {
    const filteringByYear = document.getElementById("filteringByYear");
    const inputCheckboxYearsLineChart = filteringByYear.querySelectorAll(
        "input[type='checkbox']"
    );

    let selectedYearsLineChart = [];

    inputCheckboxYearsLineChart.forEach((checkbox) => {
        checkbox.addEventListener("change", () => {
            if (checkbox.checked) {
                selectedYearsLineChart.push(checkbox.value);
            } else {
                const index = selectedYearsLineChart.indexOf(checkbox.value);
                if (index > -1) {
                    selectedYearsLineChart.splice(index, 1);
                }
            }
            displayLineChartYearlyRevenue(selectedYearsLineChart);
        });
    });

    function checkAllCheckboxesLineChart() {
        inputCheckboxYearsLineChart.forEach((checkbox) => {
    ...
    checkAllCheckboxesLineChart();

    const lineCtx = document.getElementById("myLineChart").getContext("2d");
    let myLineChart = null; // Inisialisasi variabel myLineChart

    function displayLineChartYearlyRevenue(selectedYearsLineChart) {
        console.log("Re-rendering line chart for yearly revenue");

        // Data tahunan dari JSON
        const yearlyRevenueData = [
            { year: "2015", revenue: 6273281.0 },
            { year: "2016", revenue: 6621033.0 }
        ];

        // Filter data berdasarkan tahun yang dipilih
        const filteredData = yearlyRevenueData.filter(data => selectedYearsLineChart.includes(data.year));

        const labelsYearlyRevenue = filteredData.map((data) => data.year);
        const revenueYearlyRevenue = filteredData.map((data) => data.revenue);
    ...
    // Menampilkan data 2015 dan 2016 saat halaman dimuat pertama kali
    displayLineChartYearlyRevenue(["2015", "2016"]);
});
